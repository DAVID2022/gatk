...
public final class SortSamSparkIntegrationTest extends CommandLineProgramTest {
    ...
    public void testSortBAMsSharded(
        ...
        final JavaRDD<GATKRead> reads = source.getParallelReads(actualOutputFile.getAbsolutePath(), referenceFile == null ? null : new GATKInputPath(referenceFile.getAbsolutePath()));

        final SAMFileHeader header = source.getHeader(actualOutputFile.getAbsolutePath(),
                referenceFileName == null ? null : new GATKInputPath(referenceFile.getAbsolutePath()));
